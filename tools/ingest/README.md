...
2. **All metadata is stripped, and that is verified.** Every derivative is probed
   after encoding and the run fails if any EXIF, XMP, or IPTC survived. This is not
   decorative — most phone photos carry a GPS IFD, and publishing one tells the
   internet where the shop is.
