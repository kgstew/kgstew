...
test('unrecognized disclosure level throws', () => {
...
// The YAML flow-mapping trap: `{ name: X, credit: A, B }` parses to a null-valued
// `B` key and a credit of just "A". Silently dropping the extra key is what let
// eight Fable Bound credits render wrong, so it has to be an error.
test('a collaborator with an unexpected key throws and names it', () => {
  assert.throws(
    () =>
      parse({
        collaborators: [{ name: 'Dan Mountain', credit: 'Fabrication', woodworking: null }],
      }),
    /woodworking/
  )
})

test('a collaborator keeps a credit containing commas', () => {
  const p = parse({ collaborators: [{ name: 'Bravo', credit: 'Welding, Photography' }] })
  assert.equal(p.collaborators[0].credit, 'Welding, Photography')
})
