...
const base = {
  title: 'Fable Bound',
  card: 'A viking ship that rocked when you rowed it.',
  context: 'art',
...
test('card is required to publish but not to draft', () => {
  const { card, ...noCard } = base
  assert.throws(() => parseProject(noCard, 'b', 'f.mdx', 'x'), /card/)
  assert.doesNotThrow(() => parseProject({ ...noCard, status: 'draft' }, 'b', 'f.mdx', 'x'))
})

test('an over-long card throws with its length', () => {
  assert.throws(() => parse({ card: 'x'.repeat(141) }), /141 chars/)
})
