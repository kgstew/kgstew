...
export default function Home() {
  ...
  return (
    <div className="space-y-16">
      ...
      <section>
        <h2 className="text-sm font-medium tracking-widest text-neutral-500 uppercase">Selected work</h2>
        <ul className="mt-4 divide-y divide-neutral-200">
          {featured.map((p) => (
            <li key={p.slug} className="py-4">
              <Link href={`/work/${p.slug}`} className="group block">
                <span className="font-medium group-hover:underline">{p.title}</span>
                <span className="mt-1 block text-sm text-neutral-600">{p.card}</span>
              </Link>
            </li>
          ))}
        </ul>
        <Link href="/work" className="mt-4 inline-block text-sm text-neutral-500 hover:underline">
          All work →
        </Link>
      </section>
      ...
      {posts.length > 0 && (
        <section>
          <h2 className="text-sm font-medium tracking-widest text-neutral-500 uppercase">Writing</h2>
          <ul className="mt-4 divide-y divide-neutral-200">
            {posts.map((p) => (
              <li key={p.slug} className="py-4">
                <Link href={`/writing/${p.slug}`} className="group block">
                  <span className="font-medium group-hover:underline">{p.title}</span>
                  <span className="mt-1 block text-sm text-neutral-600">{p.summary}</span>
                </Link>
              </li>
            ))}
          </ul>
